...
const Order = ({ id, category, date, partner, value, status }: Order) => {
  return (
    <div className={styles.order} style={{ border: `2px solid ${orderTypes[status ? status : category].light}` }}>
      <div className={styles.orderHeader} style={{ color: `${orderTypes[status ? status : category].light}` }}>
        <span>{id}</span>
        <span>{date}</span>
      </div>
      <div className={styles.orderPartner}>
        <span style={{ color: `${orderTypes[status ? status : category].default}` }}>
          <GoPersonFill size={12} />Cliente
        </span>
        <h4>{partner}</h4>
      </div>
      <div className={styles.orderValues}>
        <div className={styles.orderTotal}>
          <span style={{ color: `${orderTypes[status ? status : category].default}` }}>Valor do pedido</span>
          <h4 style={{ color: `${orderTypes[status ? status : category].light}` }}>{toBRL(value)}</h4>
        </div>
